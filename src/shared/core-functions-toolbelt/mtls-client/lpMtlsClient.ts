...
export class LPMtlsClient implements ILpMtlsClient {
  ...
  private async makeRequest(
    ...
    try {
      ...
      throw newLpMtlsError(
        ErrorCodes.Mtls.General,
        isError(error) ? `${error.name}: ${error.message}` : 'unknown',
      );
    }
  }
  ...
  private prepareHeader(
    ...
    for (const key in originalHeader) {
      if (
        Object.prototype.hasOwnProperty.call(originalHeader, key) &&
    ...
    if (isJson) {
      try {
        ...
        throw newLpMtlsError(
          ...
          'Provided body could not be stringified',
        );
      }
    }
  ...
  private async checkIfProcessErrorOccurred(response: Response): Promise<void> {
    ...
    const lpMessageFromTransport = response.headers.get(
      'lp-message-from-transport',
    );
    if (typeof lpMessageFromTransport === 'string') {
      const originalBody = await response.text();

      throw newLpMtlsError(
        ErrorCodes.Mtls.Remote,
        lpMessageFromTransport,
        originalBody,
      );
    }
    ...
    if (typeof wwwAuthenticate === 'string') {
      throw newLpMtlsError(
        ErrorCodes.Mtls.BadRequest,
        `Authentication to LP MTLS failed due to ${wwwAuthenticate}`,
      );
    }

    if (response?.status === 400) {
      throw newLpMtlsError(
        ErrorCodes.Mtls.Missing,
        `It seems like there was no MTLS Configuration setup for the given forward URL. Please configure MTLS Mapping.`,
      );
    }
  }
}
