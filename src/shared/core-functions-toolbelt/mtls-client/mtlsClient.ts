...
export class MTLSClient implements IMtlsClient {
   ...
   */
  constructor(private clientTLS: ClientTLS, private httpClient = fetch) {
    if (clientTLS === undefined || clientTLS === null) {
      throw newMtlsError(
        ErrorCodes.Mtls.Missing,
        'No Client TLS config was provided',
      );
    }

    const { cert, key, ca } = clientTLS;
    if (
      !this.isRightlyShapedCert(Buffer.isBuffer(cert) ? cert.toString() : cert)
    ) {
      throw newMtlsError(
        ErrorCodes.Mtls.Invalid.Cert,
        'Provided cert is not rightly shaped. Please check cert format',
      );
    }

    if (
      ca &&
      !this.isRightlyShapedCert(Buffer.isBuffer(ca) ? ca.toString() : ca)
    ) {
      throw newMtlsError(
        ErrorCodes.Mtls.Invalid.Cert,
        'Provided ca-cert is not rightly shaped. Please check ca-cert format',
      );
    }

    if (!this.isRightlyShapedKey(Buffer.isBuffer(key) ? key.toString() : key)) {
      throw newMtlsError(
        ErrorCodes.Mtls.Invalid.Key,
        'Provided key is not rightly shaped. Please check key format',
      );
    }
  }
  public async get(
      ...
      const baseConfig = {
        dispatcher: new Agent({
          connect: {
            ...
            rejectUnauthorized:
              options?.allowSelfSigned === false ? true : false,
          },
        }),
        method,
        headers,
