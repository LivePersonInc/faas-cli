...
import type { NamespaceOptions, Namespace, Entity, IContextServiceClient, ContextServiceClient } from '@liveperson/context-service';

export type ContextSessionServiceClientFactory = (url: string, apiKey: string) => ContextServiceClient;
export class FaasContextServiceClient implements IFaaSContextServiceClient {
    ...
    private async initializeClient(): Promise<void> {
        ...
        this.contextServiceClient = this.contextServiceFactory(baseUrl, this.apiKey);
    }

    public async createNamespace(namespace: string, options?: NamespaceOptions): Promise<Namespace> {
    ...
    public async setPropertiesInNamespace(namespace: string, properties: Entity, sessionId?: string): Promise<Entity> {
        await this.initializeClient();
        // @ts-ignore
        return this.contextServiceClient.setPropertiesInNamespace(this.accountId, namespace, properties, sessionId);
    }

    public async updatePropertiesInNamespace(namespace: string, properties: Entity, sessionId?: string): Promise<Entity> {
        await this.initializeClient();
        // @ts-ignore
        return this.contextServiceClient.updatePropertiesInNamespace(this.accountId, namespace, properties, sessionId);
    }

    public async getAllPropertiesInSession(namespace: string, sessionId?: string): Promise<Entity> {
        await this.initializeClient();
        // @ts-ignore
        return this.contextServiceClient.getAllPropertiesInSession(this.accountId, namespace, sessionId);
    }

    public async getSelectedPropertiesInSession(namespace: string, propertyNames: string[], sessionId?: string): Promise<Entity> {
        await this.initializeClient();
        // @ts-ignores
        return this.contextServiceClient.getSelectedPropertiesInSession(this.accountId, namespace, propertyNames, sessionId);
    }

    public async getPropertyInSession(namespace: string, propertyName: string, sessionId?: string): Promise<unknown> {
        await this.initializeClient();
        // @ts-ignore
        return this.contextServiceClient.getPropertyInSession(this.accountId, namespace, propertyName, sessionId);
    }

    public async deletePropertyInSession(namespace: string, propertyName: string, sessionId?: string): Promise<void> {
        await this.initializeClient();
        // @ts-ignore
        return this.contextServiceClient.deletePropertyInSession(this.accountId, namespace, propertyName, sessionId);
    }

    public async deleteSession(namespace: string, sessionId: string): Promise<void> {
        await this.initializeClient();
        // @ts-ignore
