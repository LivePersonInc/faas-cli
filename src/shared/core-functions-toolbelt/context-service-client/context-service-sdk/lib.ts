...
export class ContextServiceClient implements IContextServiceClient {
  ...
  /**
   * Instantiate a new client for target baseurl using provided api key
   * @param baseUrl You can use the DOMAINS constant to choose the correct baseUrl.
   * @param apiKey
   */
  constructor(
    baseUrl: string = 'DEBUG-MOCK.COM',
    apiKey: string = 'DEBUG-MOCK-API-KEY',
  ) {
  ...
  public async createNamespace(
    ...
    try {
      if (isNullOrUndefined(namespace) || isEmptyString(namespace)) {
        throw new SDKError(
          ErrorCodes.Parameter.Incorrect,
          'Namespace can not be null, undefined or empty string',
        );
      }
      ...
      if (isBuiltInNamespace(namespace)) {
        throw new SDKError(
          ErrorCodes.Parameter.Incorrect,
          'Can not create a built-in namespace',
        );
      }
      ...
      if (options?.ttl <= 0) {
        throw new SDKError(
          ErrorCodes.Parameter.Incorrect,
          'TTL needs to be a positive integer above 0',
        );
      }

      const defaultOption = {}; // Ensuring data is stored permanent
      return await this.client.createNamespace(
        accountId,
        namespace,
  ...
  public async deleteNamespace(
    ...
    try {
      ...
      if (isBuiltInNamespace(namespace)) {
        throw new SDKError(
          ErrorCodes.Parameter.Incorrect,
          'Can not delete a built-in namespace',
        );
      }
  ...
  public async setPropertiesInNamespace(
    ...
    try {
      return await this.client.setOrUpdatePropertiesInNamespace(
        accountId,
        namespace,
        properties,
        sessionId,
  ...
  public async getAllPropertiesInSession(
    ...
    try {
      return await this.client.getAllPropertiesInSession(
        accountId,
        namespace,
        sessionId,
      );
  ...
  public async getSelectedPropertiesInSession(
    ...
    try {
      return await this.client.getSelectedPropertiesInSession(
        accountId,
        namespace,
        propertyNames,
        sessionId,
  ...
  public async getPropertyInSession(
    ...
    try {
      return await this.client.getPropertyInSession(
        accountId,
        namespace,
        propertyName,
        sessionId,
  ...
  public async deletePropertyInSession(
    ...
    try {
      return await this.client.deletePropertyInSession(
        accountId,
        namespace,
        propertyName,
        sessionId,
