...
export function isV1CompatSecretObject(payload: unknown): payload is V1CompatSecretObject {
    if (isObject(payload)) {
        return 'LP_COMPAT_SECRET_TYPE' in payload && typeof payload.LP_COMPAT_SECRET_TYPE === 'string' && 'secret' in payload;
    }
    return false;
}
