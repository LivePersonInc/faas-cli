...
import {
  ...
  isFetchTypeError,
  isOAuth2ClientCreds,
  isOrchestratorErrorWithStatusCode,
  isToolbeltError,
} from '../shared/typesPredicates.js';
...
export class OrchestratorClient implements IOrchestratorClient {
  ...
  public async invoke(
    invocations: OrchestratorInvocation[],
    ...
    if (deadline > MAX_DEADLINE_TIME) {
      deadline = MAX_DEADLINE_TIME;
      console.warn(
        `Deadline exceeds maximum allowed value, ${MAX_DEADLINE_TIME}ms will be used instead`,
      );
    }
      ...
      (_, reject) =>
        (timer = setTimeout(
          ...
          newOrchestratorError(
            ErrorCodes.OrchestratorFN.Timeout,
            `Invocations timed out after a deadline of ${deadline} ms`,
          ),
        )),
    );
    ...
    try {
      const result = await Promise.race([
        this.invocationTask(invocations, finalOptions),
        deadlineInvocation,
      ]);
  ...
  private getFinalOptions(
    ...
    if (
      this.isTimeoutExceedingDeadline(
      ...
      console.warn(
        `The invocation timeout exceeds the deadline, the value of ${defaultInvocationTimeout} ms will be used`,
      );
    }
  ...
  private isTimeoutExceedingDeadline(
    timeout: number,
    deadline: number,
    totalInvocations: number,
  ): boolean {
  ...
  private async invocationTask(
    ...
    for (const invocation of invocations) {
      if (options.invokeParallel) {
        ...
        const { statusCode, body, headers, error } = await this.doInvokeRequest(
          invocation,
          options,
        );
        responses.push({
  ...
  private async doInvokeRequest(
    ...
    try {
      ...
      const willRetry =
        attempt === invocation.retries
          ? false
          : invocation.retryFunction(
              isOrchestratorErrorWithStatusCode(error)
                ? error.statusCode
                : undefined,
              error,
            );
  ...
  private async getAuthorizationHeader(): Promise<string> {
    const oauth2Creds = await this.getAppKeyCredentials(
      DEFAULT_APP_KEY_SECRET_NAME,
    );
    this.accessToken = await this.getAccessToken(oauth2Creds);
    const { access_token, token_type } = this.accessToken;
    return `${token_type} ${access_token}`;
  }
  private async getAppKeyCredentials(
    secretName: string,
    useCache: boolean = true,
  ): Promise<OAuth2ClientCreds> {
    try {
      ...
      throw newOrchestratorError(
        ...
        `Could not fetch App Key credentials from secret storage: ${
  ...
  private async getAccessToken(
    { client_id, client_secret }: OAuth2ClientCreds,
    useCache: boolean = true,
  ): Promise<AccessTokenData> {
    if (
      this.accessToken !== undefined &&
      !isAccessTokenExpired(this.accessToken) &&
      useCache
    ) {
