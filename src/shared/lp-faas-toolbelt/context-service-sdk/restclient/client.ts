...
export class RestClient implements IRestClient {
      ...
      return namespaces.map(
        (current) =>
          ({
            ...
            ...((current?.ttlSecond && {
              ttlSecond: current?.ttlSecond,
            }) as Object), // Will add property if present
          } as Namespace),
      );
    } catch (error) {
      throw await this.transform(error);
  ...
  public async getAllPropertiesInSession(
    ...
    try {
      const url = `${this.protocol}://${this.baseUrl}/v1/account/${accountId}/${namespace}/${sessionId}/properties`;
      const entitiy = await this.performRequestWithDeadline(
        'GET',
        url,
        undefined,
        {
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      try {
        return await runTaskWithDeadline(
          request(url, body, this.generateHeaders()),
          options.timeout,
        );
      } catch (error) {
        ...
        await new Promise((resolve) =>
          setTimeout(resolve, (attempt + 1) * CALL_DELAY),
        );
      }
    }
  ...
  private async transform(error: { [name: string]: unknown }): Promise<Error> {
    if (isNetworkError(error)) {
      return new NetworkError(
        (error.code || error.errno) as string,
